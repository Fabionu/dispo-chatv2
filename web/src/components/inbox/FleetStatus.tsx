          ...
          <div className="overflow-hidden rounded-panel border border-white/6 bg-white/2">
            {fleet.length === 0 ? (
              ...
              <div className="px-6 py-10 text-center">
                <p className="text-base text-muted">No vehicles match your search</p>
                <button
                  type="button"
                  onClick={() => {
                    setQuery('')
                  ...
                  className="mt-2 text-sm font-medium text-text hover:underline underline-offset-4"
                >
                  Show all vehicles
                </button>
              </div>
...
function FleetRow({ item, onOpenRoom }: { item: FleetRoom; onOpenRoom: (groupId: string) => void }) {
  return (
    <button
      ...
      className="group grid w-full grid-cols-[minmax(0,1fr)_auto] items-center gap-x-4 gap-y-1.5 px-3 py-2.5 text-left transition-colors hover:bg-white/4 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-white/20 md:grid-cols-[minmax(0,1.05fr)_minmax(0,1.25fr)_auto]"
    >
      ...
      <span className="col-span-2 min-w-0 pl-[3.125rem] md:col-span-1 md:pl-0">
        <span className="block truncate text-sm text-text">{item.tripLine}</span>
        <span className="mt-0.5 block truncate text-xs text-muted" title={item.nextLine}>
          {item.nextLine}
        </span>
      </span>
