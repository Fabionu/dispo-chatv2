    ...
    <div className="flex-1 flex flex-col min-h-0 bg-bg">
      ...
      <div className="shrink-0 px-3 pt-2.5 pb-1.5 flex flex-col gap-1.5">
        <div className="flex items-center gap-2 rounded-full border border-white/[0.06] bg-white/[0.04] px-3.5 h-10 transition-colors focus-within:border-white/[0.12]">
          {selected ? (
            <MapPin size={15} strokeWidth={1.8} className="shrink-0 text-active" />
          ) : (
            <Search size={15} strokeWidth={1.8} className="shrink-0 text-faint" />
          )}
          {selected ? (
            <>
              <span className="flex-1 truncate text-[13px]" title={selected.label}>
        ...
        {!selected && coord && (
          ...
          >
            ...
            <span className="min-w-0">
              ...
              <span className="block text-[11px] text-muted tabular-nums">{coordText(coord)}</span>
            </span>
          </button>
        )}
              ...
              >
                ...
                <span className="min-w-0">
                  ...
                  {it.label && it.label !== it.title && (
                    <span className="block text-[11px] text-muted truncate">{it.label}</span>
                  )}
                </span>
              </button>
            ))}
        ...
        {!selected && !coord && !loading && items.length === 0 && (
          <div className="text-[11.5px] text-faint px-1.5">
            {trimmed.length === 0
      ...
      <div className="flex-1 min-h-0 relative">
        ...
        />
        <div className="absolute top-2 left-1/2 -translate-x-1/2 text-[11px] text-faint bg-bg/70 backdrop-blur-sm rounded-full px-2.5 py-1 pointer-events-none">
      ...
      <div className="shrink-0 flex items-center gap-2 px-3 py-2.5">
        <div className="flex-1 min-w-0 text-[12px] text-muted truncate">
          {selected ? (
        ...
        >
          <Check size={14} strokeWidth={2.2} /> Use coordinates
        </button>
      </div>
    </div>
  )
