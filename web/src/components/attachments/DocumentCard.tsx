...
export default function DocumentCard({ name, mimeType, byteSize }: Props) {
  ...
  return (
    <div className="w-full max-w-[320px] rounded-card border border-white/[0.10] bg-surface overflow-hidden">
      {/* Preview band — a themed stand-in for the document page. */}
      <div className="relative h-[200px] bg-bg flex items-center justify-center border-b border-white/[0.06]">
        ...
        <span className="absolute top-2.5 left-2.5 rounded-chip border border-white/[0.10] bg-black/40 px-2 py-0.5 text-[10px] font-semibold tracking-wide text-muted">
          {isPdf ? 'PDF' : label}
        </span>
      </div>
      {/* Footer — filename + type · size. */}
      <div className="px-4 py-3">
        <div className="text-[13px] text-text font-medium truncate">{name}</div>
