...
function Tooltip({ label, side = 'bottom' }: { label: string; side?: TooltipSide }) {
  ...
  return (
    <span
      role="tooltip"
      className={`pointer-events-none absolute left-1/2 -translate-x-1/2 ${pos} z-10 whitespace-nowrap rounded-chip border border-white/10 bg-surface px-2 py-1 text-xs text-text opacity-0 transition-opacity duration-100 group-hover:opacity-100`}
