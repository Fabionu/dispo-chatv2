...
export default function ConfirmDialog({
  ...
  return (
    <Modal
      ...
      footer={
        <>
          <button
            onClick={onCancel}
            className="text-sm font-medium rounded-btn px-3 py-1.5 border border-white/16 text-text hover:bg-white/4 transition-colors"
          >
            {cancelLabel}
          </button>
          <button
            onClick={onConfirm}
            autoFocus
            className={`text-sm font-semibold rounded-btn px-3 py-1.5 transition-colors ${
              tone === 'alert'
    ...
    >
      <p className="text-base text-muted leading-[1.55]">{message}</p>
    </Modal>
  )
}
